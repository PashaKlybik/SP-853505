...
#define MAIN_ICON "heart.ico"
#define IDM_FILE_QUIT 3
#define START_CLICKED 4
#define STOP_CLICKED 5

bool flagOfBack = false;
int X = MAIN_WINDOW_WIDTH / 2 - 120;
int Y = MAIN_WINDOW_HEIGHT / 2 - 60;
	...
	HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case START_CLICKED:
		{
			...
			if (ret == 0)
				MessageBox(hWnd, _T("Timer is not set!"), _T("Error"), MB_OK | MB_ICONEXCLAMATION);
		}
		break;
		case STOP_CLICKED:
		{
	...
	case WM_PAINT:
	{
		const char text[] = "I`M MOVING TEXT";
		PAINTSTRUCT ps;
...
ATOM RegMyWindowClass(HINSTANCE hInst, LPCTSTR lpzClassName)
{
	...
	wcWindowClass.hCursor = (HICON)LoadImage(NULL, _T("./cursor.ico"), IMAGE_CURSOR, 20, 20, LR_LOADFROMFILE);
	wcWindowClass.hIcon = (HICON)LoadImage(NULL, _T("./heart.ico"), IMAGE_ICON, 0, 0, LR_LOADFROMFILE);
	...
	int nCmdShow)
{
	...
	HWND hWnd = CreateWindow(lpzClass, _T("LAB1"),
